...
async function postStrategyCycleWorkerKick(): Promise<void> {
  ...
  if (!res.ok) {
    ...
    console.warn("[okr-contribution-assessment] worker kick failed", res.status, body.slice(0, 300));
  }
}
...
function scheduleStrategyCycleWorkerKick(): void {
  ...
  after(async () => {
    try {
      ...
      console.warn("[okr-contribution-assessment] worker kick error", e);
    }
  });
}
...
/**
 * Plant einen Background-Job zur OKR-Contribution-Bewertung (LLM), sofern Tenant-Flag und Budgetrahmen passen.
 */
export async function scheduleOkrContributionAssessmentJob(input: {
  supabase: Supabase;
  organizationId: string;
  ...
  const { error } = await input.supabase.schema("app").from("analysis_background_jobs").insert({
    ...
    job_type: "okr_contribution_assessment",
    status: "pending",
    payload: {
  ...
  if (error) {
    console.error("[scheduleOkrContributionAssessmentJob]", error.message);
    return;
  }
