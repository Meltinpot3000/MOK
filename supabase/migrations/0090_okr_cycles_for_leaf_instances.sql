-- migrate:up
-- OKR-Zeitraeume (app.okr_cycles): pro Blatt-Instanz im Zyklusbaum automatisch anlegen.
-- Bisher erzeugte regenerate_cycle_instances nur cycle_instances; ohne okr_cycles bleibt
-- das OKR-Dashboard leer.
...
-- Bestehende Blatt-Instanzen: fehlende OKR-Zeitraeume nachziehen
insert into app.okr_cycles (
  organization_id,
  cycle_instance_id,
  name,
...
select
  ...
  ci.legacy_planning_cycle_id
from app.cycle_instances ci
where ci.legacy_planning_cycle_id is null
  and not exists (
    select 1
    from app.cycle_instances child
    where child.parent_instance_id = ci.id
...
-- migrate:down
-- Kein vollstaendiger Rollback: okr_cycles-Zeilen und Function-Definition bleiben;
-- bei Bedarf Migration manuell rueckgaengig machen.
select 1;
